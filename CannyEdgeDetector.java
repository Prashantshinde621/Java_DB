...
public class CannyEdgeDetector {
    ...
    // fields
    public static countCell obj = null;
    private int height;
    private int width;
    private int picsize;
    private int[] data;
        ...
        for (int x = initX; x < maxX; x++) {
            for (int y = initY; y < maxY; y += width) {
                ...
                if (xGrad * yGrad <= (float) 0 /*(1)*/
                        ...
                        ? (tmp = Math.abs(xGrad * gradMag)) >= Math.abs(yGrad * neMag - (xGrad + yGrad) * eMag) /*(3)*/ && tmp > Math.abs(yGrad * swMag - (xGrad + yGrad) * wMag) /*(4)*/
                        : (tmp = Math.abs(yGrad * gradMag)) >= Math.abs(xGrad * neMag - (yGrad + xGrad) * nMag) /*(3)*/ && tmp > Math.abs(xGrad * swMag - (yGrad + xGrad) * sMag) /*(4)*/
                        : Math.abs(xGrad) >= Math.abs(yGrad) /*(2)*/
                        ? (tmp = Math.abs(xGrad * gradMag)) >= Math.abs(yGrad * seMag + (xGrad - yGrad) * eMag) /*(3)*/ && tmp > Math.abs(yGrad * nwMag + (xGrad - yGrad) * wMag) /*(4)*/
                        : (tmp = Math.abs(yGrad * gradMag)) >= Math.abs(xGrad * seMag + (yGrad - xGrad) * sMag) /*(3)*/ && tmp > Math.abs(xGrad * nwMag + (yGrad - xGrad) * nMag) /*(4)*/) {
                    magnitude[index] = gradMag >= MAGNITUDE_LIMIT ? MAGNITUDE_MAX : (int) (MAGNITUDE_SCALE * gradMag);
                    //NOTE:this point as: Math.atan2(yGrad, xGrad);
                } else {
                    magnitude[index] = 0;
                }
            }
...
//adjust its parameters as desired
            ...
            BufferedImage frame = ImageIO.read(new File("c:\\blood4.jpg"));
            System.out.println("1");
            ...
            byte[][] data = new byte[edges.getWidth()][edges.getHeight()];
            ImageIO.write(edges, "png", new File("c:\\images\\output.png"));
            for (int i = 0; i < edges.getWidth(); i++) {
                for (int j = 0; j < edges.getHeight(); j++) {
                    int p = edges.getRGB(i, j);
                    int r = (p & 0xff0000) >> 16;
                    int g = (p & 0xff00) >> 8;
                    int b = p & 0xff;
                    int y = (int) (0.3 * r + 0.59 * g + 0.11 * b);
                    data[i][j] = (byte) (y == 255 ? 1 : 0);
            ...
            obj = new countCell();
            obj.countCell(data);
            System.out.println("Total Cell count = " + obj.getCellcount() + " , " + 
            "Total Wbc count = " + obj.getWbc());
        } catch (IOException ex) {
            Logger.getLogger(CannyEdgeDetector.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static countCell getWBCCount(BufferedImage img) {
        try {
            //create the detector
            CannyEdgeDetector detector = new CannyEdgeDetector();
//adjust its parameters as desired
            ...
            obj.countCell(data);
            System.out.println("------------------------------------------------------");
            System.out.println(obj.getCellcount() + " " + obj.getWbc());
            System.out.println("------------------------------------------------------");
            return obj;
        } catch (IOException ex) {
            Logger.getLogger(CannyEdgeDetector.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public int countallCircle(byte[][] data) {
        int cnt = 0;
